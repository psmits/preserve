...
Survival analysis is complicated by censored observations where the ultimate time of extinction for some taxa could not be observed during the study window.

% lets try fully observed vs fully observed + censored?
...
Comparisons between the observed distribution of durations to the distrubtions of 100 simulated datasets reveals the heterogeneous fit of the model to the data (Fig. \ref{fig:dens_overlay_zoom}). The two major aspects of misfit that are observable are at at durations between 1-3 stages and 8-9 stages. In both of these cases, the observed distribution is effectively left-shifted e.g. the shared peak from the 100 simulated datasets is at 1 while that from the observed is at 2. However, our goal of our model is estimating the expected duration of a genus given its covariate information. While the model estimates are not exact, it is possible that our model fits the ``middle'' of our data well but does poorly towards the tails. 
...
Comparisons between the survival functions estimated from the empirical data and from 100 simulated datasets further expands the picture of model (mis)fit (Fig. \ref{fig:surv}). The survival curves of the 100 simulated datasets are very close to the survival function estimated from the empirical data for very short durations and high durations. For intermediate durations (\(\sim\)4-20 stages), our estimates are right shifted meaning that our estimated survival probabilities are possibly too high. What is interesting about this misfit is that the functions from empirical datasets and from the simulated datasets diverge when genera have a less than 50\% probability of surviving.
...
The summary statistic based posterior predictive checks reveal a similar pattern of heterogeneity previously observed. For nearly every origination cohort, the model is able to approximately recapitulate the observed mean duration (Fig. \ref{fig:group_mean}). There are a few exceptions to this pattern; for example, the model overestimates the mean taxon duration for the Sakmarian, Artinskian, and Wordian. 

In comparison, the model has a much more heterogeneous adequacy of fit to origination cohort median taxon duration (Fig. \ref{fig:group_median}). This result stands in contrast to the models adequacy in predicting the overall median taxon duration (Fig. \ref{fig:ppc_median}). Cohorts with notably inadequate model predictions are the Hirnantian, Emsian, Eifelian, Giventian, Asselian, Sakmarian, Roadian, among others. Interestingly, model misfit is in both directions, with some cohorts underestimating median duration while others overestimate median duration.
