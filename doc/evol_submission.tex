...
%   cor gap and rel abundance
%   using relative abundance as measure of sampling
% if wanting to use gap statistic
%   imputed model
%   truncated model
...
Sampling was found to have a negative effect on duration: greater sampling, shorter duration (Table \ref{tab:param}). While this affect appears large compared to the other regression coefficients, this is only because sampling was not standardized like the other covariates. To make the coefficients comparable, \(\mu_{r}\) is multiplied by twice the standard deviation of geographic range which yields mean -1.51 (\(\pm 0.24\) SD), which has a 99.25\% probability of being greater than the estimate of \(\delta\). The effect of sampling, however, has a 96.8\% probability of being greater in magnitude than the effect of environmental preference \(\mu_{v}\), while there is a 0\% probability that \(\delta\) is greater in magnitude than \(\mu_{v^{2}}\)..
