...
In order to test for an association between extinction intensity and extinction selectivity, both extinction rate and trait-based differences in extinction rate have to be estimated. Payne/Wang approach is to estimate this separately for each time point and then compare. My approach is to write one model to estimate all of this simultaneously and take advantage of Bayesian things like partial pooling and regularization to smooth over issues of small sample sizes and weak effects. Mine has the overall advantage of putting everything on the same scale and more easily comparable. I also get probabilities at the end which are so much easier to read than p-values.





Additionally, in order to test the proposed macroevolutionary mechanism behind the \citet{Jablonski1986} scenario; not only do the taxon trait effects needs to be modeled, but the correlation between trait effects need to be modeled as well. 
