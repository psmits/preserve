...
\subsection{Environmental preference}

Environmental preference was estimated as the log-odds of occuring in an epicontinental environment. This estimate is drawn from a hierarchical binomial distribution as esimated from all taxa originating in the same cohort. The resulting probability of occuring in an epicontinental environment \(\theta\) is then logit transformed. This model of environmental occurrence is defined as

\begin{equation}
  \begin{aligned}
    e_{i} &\sim \mathrm{Binomial}(o_{i}, \theta_{c[i]}) \\
    \theta_{c} &\sim \mathrm{logit}^{-1}(\mathcal{N}(\mu_{c}, \sigma_{c}) \\
    \mu ~ \mathcal{N}(0, 1) \\
    \sigma ~ \mathrm{C}^{+}(1)
  \end{aligned}
  \label{eq:env_prob}
...
For an explanation of how this model was developed, parameter explanations, and choice of priors, please see Appendix \ref{sec:survival}. Note that this model (Eq. \ref{eq:wei_total}) do not include how the uncertainty in environmental affinity is included nor how censored observations are included. For an explanation of both of these aspects, see Appendices \ref{sec:uncer} and \ref{sec:cen}.
