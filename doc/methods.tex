...
\begin{equation}
  \begin{aligned}
    k_{i} &\sim \text{Binomial}(o_{i}, \theta_{c[i]}) \\
    \theta_{c} &\sim \text{logit}^{-1}(\mathcal{N}(\mu_{c}, \sigma_{c}) \\
    \mu ~ \mathcal{N}(0, 1) \\
    \sigma_ ~ \text{C}^{+}(1)
  \end{aligned}
  \label{eq:env_prob}
...
where \(k_{i}\) is the number of epicontinental occurrences for species \(i\), and \(o_{i}\) is the total number of occurrences for species \(i\).
