...
To analyze a potential association between extinction intensity and extinction selectivity, average extinction rate and the trait-based differences in extinction rate need to be estimated. In particular, how these values vary over time needs to be estimated; this is a data intensive and parameter rich requirement. In general, there are two approaches to this problem, which I will call phylogenetic comparative (PCM) or paleobiological. PCM approaches uses branch length information from a time-scaled phylogeney in order to estimate the speciation and extinction rates from a birth-death model that best fits the phylogeny's topology \citep{Maddison2007,Fitzjohn2010,Goldberg2011a,Goldberg2005,Rabosky2013,Stadler2013b,Stadler2011,Stadler2013a} MORE CITATION. In contrast, the paleobiological approach to estimating extinction rates, as well as trait-dependent differences in those rates, is a type of time-series analysis where the occurrence and sampling histories of fossil taxa are used to estimated the underlying rates of a birth-death process (with incomplete observation) that fits the observed data.
