...
\(y_{i}\) is the duration of genus \(i\) in geologic stages, \(X\) is a matrix of covariates including a constant term, \(B_{j}\) is the vector of regression coefficients for origination cohort \(j\), \(\Sigma\) is the covariance matrix of the regression coefficients, \(\tau\) is a vector of scales the standard deviations of the between-cohort variation in regression coefficient estimates, and \(\Omega\) is a correlation matrix of the regression coefficients.
...
\begin{equation}
  ...
  \label{eq:wei_total}
\end{equation}
The principle difference between this model and the previous (Eq. \ref{eq:exp_total}) is the inclusion of the shape parameter \(\alpha\). Note that \(\sigma\) is approximately \(1 / \lambda\).
