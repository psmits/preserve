...
\begin{frame}
  \begin{block}{Selection on species traits}
    \begin{itemize}
      \item Extinction is second only to speciation in describing the diversifiction process \\(Raup 1994 \textit{PNAS}, Stanley 1975 \textit{PNAS}).
      \item A species with a beneficial trait should persist for longer, on average, than a species without that beneficial trait \\(Jablonski 2008 \textit{Paleobio}, Rabosky and McCune 2010 \textit{TREE}).
      \item Taxon survival an aspect of \alert{taxon fitness} \\(Cooper 1984 \textit{J. Theo. Biol.}, Palmer and Feldman 2012 \textit{PLoS One}).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}
  \begin{block}{Trait-dependent extinction}
    \begin{itemize}
      ...
      \item difficult to estimate from phylogenies alone \\(Rabosky 2010 \textit{Evolution}, Liow et al \textit{Syst Biol}, Quental and Marshall 2008 \textit{Evolution})
      \item fossil record is (imperfect) observation of time, speciation, extinction
    \end{itemize}
  \end{block}
\end{frame}
